...
impl TaskEncoder for MirPureEnc<Impure> {
    task_encoder::encoder_cache!(MirPureEnc<Impure>);
    ...
    fn do_encode_full<'vir>(
        ...
        let expr = vir::with_vcx(move |vcx| {
            ...
            let expr_inner =
                Enc::<Impure>::new(vcx, task_key.0, def_id, kind, &body, deps).encode_body()?;
...
impl TaskEncoder for MirPureEnc<Purified> {
    task_encoder::encoder_cache!(MirPureEnc<Purified>);
    ...
    fn do_encode_full<'vir>(
        ...
        let expr = vir::with_vcx(move |vcx| {
            ...
            let expr_inner =
                Enc::<Purified>::new(vcx, task_key.0, def_id, kind, &body, deps).encode_body()?;
            ...
            let expr = vcx.mk_lazy_expr(
                vir::vir_format!(vcx, "pure body {def_id:?}"),
                deps.require_ref::<TyUsePurifiedEnc>(ret)?.snapshot,
                Box::new(move |vcx, lctx: ExprInput<'_>| {
                    // check: are we actually providing arguments for the
                    //   correct `DefId`?
                    assert_eq!(lctx.0, def_id);
...
struct Enc<'vir: 'enc, 'enc, P: NotPure>
where
    MirPureEnc<P>: TaskEncoder,
{
    vcx: &'vir vir::VirCtxt<'vir>,
    encoding_depth: usize,
    def_id: DefId,
    ...
    deps: &'enc mut TaskEncoderDependencies<'vir, MirPureEnc<P>>,
...
impl<'vir: 'enc, 'enc> PureRvalueEnc<'vir> for Enc<'vir, 'enc, Impure> {
    type Encoder = MirPureEnc<Impure>;
...
impl<'vir: 'enc, 'enc> PureRvalueEnc<'vir> for Enc<'vir, 'enc, Purified> {
    type Encoder = MirPureEnc<Purified>;
    type EncodePlaceCtxt = HashMap<mir::Local, Version<'vir>>;
    type ExprCurr = ExprInput<'vir>;
    type ExprNext = vir::ExprKind<'vir>;
...
impl<'vir: 'enc, 'enc> Enc<'vir, 'enc, Impure> {
    fn new(
        vcx: &'vir vir::VirCtxt<'vir>,
        encoding_depth: usize,
        def_id: DefId,
        kind: PureKind,
        body: &'enc mir::Body<'vir>,
        deps: &'enc mut TaskEncoderDependencies<'vir, MirPureEnc<Impure>>,
    ) -> Self {
        assert!(
        ...
        let local_defs = deps
            .require_dep::<MirLocalDefEnc<Impure>>(local_def_enc_task)
            .unwrap();
        Self {
            vcx,
            encoding_depth,
    ...
    fn mk_phi_acc<T: vir::CompType>(
        &mut self,
        tuple_ref: &crate::encoders::ViperTupleEncOutput<'vir, Pure>,
        phi_idx: Version<'vir>,
        elem_idx: usize,
        _ty: vir::Type<'vir, T>,
    ) -> ExprRet<'vir> {
    ...
    fn encode_body(&mut self) -> Result<ExprRet<'vir>, EncodeFullError<'vir, MirPureEnc<Impure>>> {
    ...
    fn encode_cfg(
    ...
    ) -> Result<Option<Update<'vir>>, EncodeFullError<'vir, MirPureEnc<Impure>>> {
        if curr == join_point {
        ...
        match &term.kind {
            ...
            mir::TerminatorKind::SwitchInt { discr, targets } => {
                ...
                let tuple_ref = self
                    .deps
                    .require_dep::<ViperTupleEnc<Pure>>((self.def_id, mod_tys))
                    .unwrap();
    ...
    fn encode_stmt(
        ...
        location: mir::Location,
    ) -> Result<Update<'vir>, EncodeFullError<'vir, MirPureEnc<Impure>>> {
    ...
    fn encode_rvalue(
        ...
        span: Span,
    ) -> Result<ExprRet<'vir>, EncodeFullError<'vir, MirPureEnc<Impure>>> {
    ...
    fn encode_prusti_builtin(
        ...
        Ok(match builtin {
            ...
            PrustiBuiltin::Forall | PrustiBuiltin::Exists => {
                ...
                let body = self
                    .deps
                    .require_dep::<MirPureEnc<Impure>>(MirPureEncTask {
...
impl<'vir: 'enc, 'enc> Enc<'vir, 'enc, Purified> {
    fn new(
        vcx: &'vir vir::VirCtxt<'vir>,
        encoding_depth: usize,
        def_id: DefId,
        kind: PureKind,
        body: &'enc mir::Body<'vir>,
        deps: &'enc mut TaskEncoderDependencies<'vir, MirPureEnc<Purified>>,
    ) -> Self {
        assert!(
            !graph::is_cyclic(&body.basic_blocks),
            "MIR pure encoding does not support loops"
        ...
        let local_defs = deps
            .require_dep::<MirLocalDefEnc<Purified>>(local_def_enc_task)
            .unwrap();
        Self {
            vcx,
            encoding_depth,
    ...
    fn ty_use(&mut self, ty: ty::Ty<'vir>) -> TyUsePurified<'vir> {
        let ty_task = RustTyDecomposition::from_ty(ty, self.vcx.tcx(), self.context);
    ...
    fn get_ty_for_local(&mut self, local: mir::Local) -> vir::TypeSnap<'vir> {
        ...
        let ty_task = RustTyDecomposition::from_ty(ty, self.vcx.tcx(), self.context);
        self.deps
            .require_ref::<TyUsePurifiedEnc>(ty_task)
            .unwrap()
            .snapshot
    }
    ...
    fn mk_phi_acc<T: vir::CompType>(
        &mut self,
        tuple_ref: &crate::encoders::ViperTupleEncOutput<'vir, Purified>,
        phi_idx: Version<'vir>,
        elem_idx: usize,
        _ty: vir::Type<'vir, T>,
    ) -> ExprRet<'vir> {
    ...
    fn encode_body(
        &mut self,
    ) -> Result<ExprRet<'vir>, EncodeFullError<'vir, MirPureEnc<Purified>>> {
        let mut init = Update::new();
        let v0 = Version::default();
        // TODO: what about locals which never have StorageLive (i.e. always_live)?
        init.versions.insert(mir::RETURN_PLACE, v0);
    ...
    fn encode_cfg(
        ...
        curr: mir::BasicBlock,
        join_point: mir::BasicBlock,
    ) -> Result<Option<Update<'vir>>, EncodeFullError<'vir, MirPureEnc<Purified>>> {
        if curr == join_point {
            // We are done with the current fragment of the CFG, the rest is
            // handled in a parent call.
            return Ok(Some(Update::new()));
        ...
        match &term.kind {
            ...
            mir::TerminatorKind::SwitchInt { discr, targets } => {
                ...
                let tuple_ref = self
                    .deps
                    .require_dep::<ViperTupleEnc<Purified>>((self.def_id, mod_tys))
                    .unwrap();
                let otherwise_update = updates.pop().unwrap();
                let phi_expr = targets.iter().zip(updates).fold(
                    self.reify_branch(&tuple_ref, &mod_locals, &new_curr_ver, otherwise_update),
    ...
    fn encode_stmt(
        ...
        stmt: &mir::Statement<'vir>,
        location: mir::Location,
    ) -> Result<Update<'vir>, EncodeFullError<'vir, MirPureEnc<Purified>>> {
        let mut update = Update::new();
        match &stmt.kind {
            &mir::StatementKind::StorageLive(local) => {
                let new_version = self.bump_version_no_assign(local, location);
    ...
    fn encode_place_element(
        ...
        match elem {
            mir::ProjectionElem::Deref => {
                ...
                match place_ty.ty.kind() {
                    TyKind::Adt(adt, _) if adt.is_box() => {
                        let e_ty = self.deps.require_dep::<TyUsePurifiedEnc>(ty_task).unwrap();
                        let proj =
                            e_ty.expect_variant_opt(place_ty.variant_index)[abi::FieldIdx::ZERO];
                        let proj_app = proj.field_snap(encoded_place.snap.downcast_ty());
                        EncodedPlace::new(proj_app, None)
                    }
                    TyKind::Ref(.., ty::Mutability::Not) => {
                        let e_ty = self
                            .deps
                            .require_dep::<TyUsePurifiedEnc>(ty_task)
                            .unwrap()
                            .expect_immref();
                        let val_expr = e_ty.value_access(encoded_place.snap.downcast_ty());
                        EncodedPlace::new(val_expr, encoded_place.place_ref)
