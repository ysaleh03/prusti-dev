use crate::encoders::{
    Impure, ImpureEncVisitor, MirLocalDefEncOutput, MirSpecEnc, Pure,
...
impl<'vir> WandEncOutput<'vir> {
    ...
    fn encode_predicates_for_function_shape_node(
        ...
        let predicates = deps
            .require_dep::<IndirectPredicatesEnc<Pure>>(g.with_base(ty))
            .unwrap()
...
impl TaskEncoder for WandEnc {
    ...
    fn do_encode_full<'vir>(
        ...
        vir::with_vcx(|vcx| {
            ...
            let spec = deps.require_dep::<MirSpecEnc<Impure>>((def_id, def_id, false))?;
