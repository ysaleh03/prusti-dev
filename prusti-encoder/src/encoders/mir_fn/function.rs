...
impl TaskEncoder for FunctionCallEnc<Impure> {
    ...
    fn do_encode_full<'vir>(
        ...
        let function_ref = deps.require_ref::<FunctionEnc<Impure>>(task_key.callee)?;
    ...
    fn emit_outputs<'vir>(program: &mut task_encoder::Program<'vir>) {
        FunctionEnc::<Impure>::emit_outputs(program);
    }
}
...
impl TaskEncoder for FunctionCallEnc<Purified> {
    ...
    fn do_encode_full<'vir>(
        ...
        deps.emit_output_ref(*task_key, ())?;
        let function_ref = deps.require_ref::<FunctionEnc<Purified>>(task_key.callee)?;
        let signature = RustSignature::new(task_key.callee);
        let ty_args = deps.require_dep::<GArgsTyEnc>(task_key.gargs)?;
        let inputs = signature
            .inputs
    ...
    fn emit_outputs<'vir>(program: &mut task_encoder::Program<'vir>) {
        FunctionEnc::<Purified>::emit_outputs(program);
    }
}
...
impl TaskEncoder for FunctionEnc<Impure> {
    task_encoder::encoder_cache!(FunctionEnc<Impure>);
...
impl TaskEncoder for FunctionEnc<Purified> {
    task_encoder::encoder_cache!(FunctionEnc<Purified>);
    type TaskDescription<'tcx> = DefId;
