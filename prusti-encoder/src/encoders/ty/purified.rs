...
impl TaskEncoder for TyPurifiedEnc {
    ...
    fn do_encode_full<'vir>(
        ...
        vir::with_vcx(|vcx| {
            let mut builder = TyPurifiedBuilder::new(deps, vcx, task_key);
...
#[repr(transparent)]
pub(crate) struct DomainBuilder<'vir>(TyPurifiedBuilder<'vir>);
...
impl<'vir> Deref for DomainBuilder<'vir> {
    type Target = TyPurifiedBuilder<'vir>;
...
#[repr(transparent)]
pub(crate) struct AdtBuilder<'vir>(TyPurifiedBuilder<'vir>);
...
pub(crate) struct TyPurifiedBuilder<'vir> {
...
impl<'vir> TyPurifiedBuilder<'vir> {
