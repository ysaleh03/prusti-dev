...
use vir::{
    AdtDestructorPSnap, AdtDestructorRef, Arity, CallableIdn, CastType, CompType, DomainAxiomData,
    DomainIdnCSnap, FunctionIdn, Type,
};
    ...
    fn do_encode_full<'vir>(
        ...
        vir::with_vcx(|vcx| {
            ...
            let (specifics, builder) = match task_key.kind() {
                ...
                | TyKind::Int(_)
                | TyKind::Uint(_)
                | TyKind::Float(_) => super::kinds::primitive::domain(*task_key, deps, builder)?,
                TyKind::Closure(..) => {
                ...
                TyKind::Ref(_, _, ty::Mutability::Mut) => {
                    super::kinds::mutref::domain(*task_key, &output_ref, deps, builder)?
                }
    ...
    pub(crate) fn build(builder: PureTypeBuilder<'vir>) -> DomainEncOutput<'vir> {
        match builder {
            ...
            Ok(builder) => {
                ...
                let adt = builder.vcx.mk_adt(
                    domain_ident.name(),
                    &[],
                    builder.vcx.alloc_slice(builder.constructors.as_slice()),
                );
...
impl<'vir> AdtBuilder<'vir> {
    ...
    pub(crate) fn constructor<A: vir::Arity>(
        ...
        self.constructors
            .push(self.vcx.mk_adt_constructor(name, locals));
    ...
    pub(crate) fn build_discr_fn(
        ...
        let built_fn = self
            .vcx
            .mk_function(ident, (param,), &[], &[], None, Some(acc));
