use crate::{
    encoders::{
        domain::{
            DomainBuilder, DomainDataPurifiedImmRef, DomainEnc, DomainEncOutputRef,
            DomainEncSpecifics,
        },
        predicate::{PredicateBuilder, PredicateEncData, PredicateEncDataPurifiedImmRef},
...
pub(crate) fn domain<'vir>(
    ...
    builder.axiom("typeof", vir::expr! {
        forall p: [inner_type] ::
            {[output_ref.typeof_function]([cons_ident](p))}
            ([output_ref.typeof_function]([cons_ident](p))) == ([ty_type_func.ty_constructor]([generic_enc.param_type_function](p)))
    });
    let generic_enc = deps.require_ref::<GenericEnc>(())?;
    ...
    */
    ...
    Ok(DomainEncSpecifics::PurifiedImmRef(
        DomainDataPurifiedImmRef {
...
> {
    ...
    let snap_data = snap.specifics.expect_purified_immref();
    ...
    Ok((
        PredicateEncData::PurifiedImmRef(PredicateEncDataPurifiedImmRef {
